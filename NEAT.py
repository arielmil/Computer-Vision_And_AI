...
class Genome:
    ...
    def decode_genome_to_torch(self):
        ...
        # Pegamos as referências para usar na classe interna
        genome = self.genome
        config = self.config
        topo_order = self.get_topological_order()

        class NeatModule(nn.Module):
            def __init__(_self, genome, config, topo_order):
                super().__init__()
                ...
                _self.num_inputs = self.config.genome_config.num_inputs
                _self.num_outputs = self.config.genome_config.num_outputs
            ...
            def forward(_self, x):
                ...
                # Copia as inputs:
                for i_in in range(_self.num_inputs):
                    idx = _self.node_id_to_idx[i_in]
                    activations[:, idx] = x[:, i_in]

                adjacency = {}
                for n_id in _self.node_order:
                    adjacency[n_id] = []
                ...
                for node_id in _self.node_order[_self.num_inputs:]:
                    ...
                    # Exemplo: softmax
                    node_out = F.softmax(total_in)
                    activations[:, node_idx] = node_out

                # Extrair outputs
                outs = []
                for i_out in range(_self.num_outputs):
                    out_node_id = _self.num_inputs + i_out
                    out_idx = _self.node_id_to_idx[out_node_id]
                    outs.append(activations[:, out_idx])
                return torch.stack(outs, dim=1)

        return NeatModule(genome, config, topo_order)
